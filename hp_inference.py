...
def load_pickle(pickle_path):
    ...
    output_bias = variables['hp_model'].output_bias
    model_inputs = variables['hp_model']

    return output_bias, val_idx, model_inputs
...
def load_tf_model(data_folder, model_name, cp_folder, inputs):
...
def inference(model, x_test, test_idx, train_X, train_Y, model_inputs):
    # Performance metric
    ...
    val_set = val_set.map(lambda image, label: preprocess_dataset.preprocess(image, label, 42, model_inputs,
                                                                             training=False))
    ...
    for count, idx in enumerate(test_idx):
        ...
        test_set = test_set.map(lambda image, label: preprocess_dataset.preprocess(image, label, 42, model_inputs,
                                                                                   training=False))
...
MODEL = 'EfficientNetB0_0'
PICKLE_PATH = r'C:\Kaggle\RainForest_R0\HPTuning\224_512\EfficientNetB0\EfficientNetB0_0\EfficientNetB0_0.pickle'
...
val_scores = []
test_prediction = np.empty([test_X.shape[0], 24], dtype=np.float32)
for i, model_name in enumerate(list_of_models):
    print(f'Inference for Model: {model_name}')
    output_bias, val_idx, model_inputs = load_pickle(PICKLE_PATH)
    model = load_tf_model(DATA_FOLDER, MODEL, model_name, model_inputs)

    # Inferences on Test Dataset
    # TODO add validation to inference (put idx in folder with model as pickle)
    test_predict_single_model, val_score = inference(model, test_X, test_idxs,
                                                     train.X[val_idx[i]], train.Y[val_idx[i]],
                                                     model_inputs)
